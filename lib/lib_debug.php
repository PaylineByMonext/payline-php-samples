...
if ( !defined('__LIB_DEBUG_PHP') )
{
	...
	************************************************/
    ...
    function insertTransactionData($contractNumber,$orderRef,$transactionId,$paymentAction,$amount,$currency,$resultCode,$token=null){
		if(defined('DB_HOST')){
			...
			if(!$link){
				...
				if($token){
					$query = "SELECT 1 FROM `transaction` WHERE `TOKEN` = '$token'";
					$result = mysql_query($query, $link);
					$row = mysql_fetch_assoc($result);
					if($row){
						return 2; // le token est en base
					}
				}
				$query = "INSERT INTO `transaction`(`ENVIRONMENT`, `MERCHANT_ID`, `CONTRACT`, `ORDER_REF`, `TRANSACTION_ID`, `ACTION`, `TOKEN`, `AMOUNT`, `CURRENCY`, `RESULT`, `DATE`)
							 VALUES ('".ENVIRONMENT."','".MERCHANT_ID."','$contractNumber','$orderRef','$transactionId','$paymentAction','$token','$amount','$currency','$resultCode',now())";
				$result = mysql_query($query, $link);
				if (!$result) {
	...
	function insertBillingRecordData($contractNumber,$walletId,$paymentRecordId,$billingRecordId){
	    if(defined(DB_HOST)){
    	    $link = mysql_connect(DB_HOST, DB_OWNER, DB_PASS);
    	    if(!$link){
    	        return false;
    	    }else{
    	        $query = "INSERT INTO `billingrecord`(`ENVIRONMENT`, `MERCHANT_ID`, `CONTRACT`, `WALLET_ID`,`PAYMENT_RECORD_ID`, `BILLING_RECORD_ID`, `DATE`)
    			 VALUES ('".ENVIRONMENT."','".MERCHANT_ID."','$contractNumber','$walletId','$paymentRecordId','$billingRecordId',now())";
    	        mysql_select_db(DB_NAME, $link);
    	        $result = mysql_query($query, $link);
    	        if (!$result) {
    	            return mysql_error($link);
	...
	function insertPaymentRecordData($contractNumber,$walletId,$paymentRecordId){
	    if(defined(DB_HOST)){
    	    $link = mysql_connect(DB_HOST, DB_OWNER, DB_PASS);
    	    if(!$link){
    	        return false;
    	    }else{
    	        $query = "INSERT INTO `paymentrecord`(`ENVIRONMENT`, `MERCHANT_ID`, `CONTRACT`, `WALLET_ID`,`PAYMENT_RECORD_ID`, `DATE`)
    			 VALUES ('".ENVIRONMENT."','".MERCHANT_ID."','$contractNumber','$walletId','$paymentRecordId',now())";
    	        mysql_select_db(DB_NAME, $link);
    	        $result = mysql_query($query, $link);
    	        if (!$result) {
    	            return mysql_error($link);
			...
			$handle = fopen($paymentRecordsFile, 'a+');
			fwrite($handle, 'ENVIRONMENT='.$_SESSION['ENVIRONMENT'].'#'.'MERCHANT_ID='.$_SESSION['MERCHANT_ID']."#CONTRACT=$contractNumber#WALLET_ID=$walletId#PAYMENT_RECORD_ID=$paymentRecordId#DATE=".time()."#\n");
			fclose($handle);
			return 1;
	    }
	}
